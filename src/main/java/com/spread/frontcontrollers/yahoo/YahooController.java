package com.spread.frontcontrollers.yahoo;
...
public class YahooController implements Serializable {
	
	private static final long serialVersionUID = 702182050599052543L;

	private static final Logger logger = LogManager.getLogger(YahooController.class);
	
	@Autowired
	@Qualifier("yahooFetcher")
	private SearchEngineFetcher yahooFetcher;
	...
	public ModelAndView search(@RequestParam("query")String query) throws Exception {
		...
		ModelAndView model = new ModelAndView("yahoo_index");
		...
		SearchResult searchResult = yahooFetcher.fetch(query, false);
